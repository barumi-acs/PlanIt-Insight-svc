...
```
Connection refused: getsockopt: localhost/127.0.0.1:9092
```
...
grpc:
  ...
  client:
    chat-service:
      address: ${GRPC_CHAT_SERVICE_ADDRESS:static://localhost:50051}
      # ...
```
...
| 9092 | gRPC | ActionLog 서버 (Schedule-svc → Insight-svc) | `GRPC_SERVER_PORT` |
...
│ - HTTP: 8082                                                │
│ - gRPC Server: 9091                                         │
│ - gRPC Client → Insight-svc:9092 (ActionLog 기록)          │
└─────────────────────────────────────────────────────────────┘
                    ↓ gRPC 호출 (9092)
┌─────────────────────────────────────────────────────────────┐
│ Insight-svc                                                 │
│ - HTTP: 8084                                                │
│ - gRPC Server: 9092 (ActionLog 서비스)                     │
│ - gRPC Client → InsightAI-svc:50051 (Chatbot, Report)      │
└─────────────────────────────────────────────────────────────┘
                    ↓ gRPC 호출 (50051)
┌─────────────────────────────────────────────────────────────┐
│ InsightAI-svc                                               │
│ - HTTP: 8085 (FastAPI)                                     │
│ - gRPC Server: 50051 (Chatbot + Report)                    │
└─────────────────────────────────────────────────────────────┘
```
...
시작 로그 확인:
```
gRPC Server started on port 9092
...
연결 성공 로그:
```
gRPC client connected to insight-service: static://localhost:9092
```
...
3. **방화벽 확인**
   - Windows Defender 방화벽에서 9092 포트 허용

4. **환경 변수 확인**
   ```bash
   # Schedule-svc application.yml
   insight-service:
     address: static://localhost:9092  # 포트 확인!
   ```
